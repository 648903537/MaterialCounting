...
namespace com.amtec.action
{
    public class CheckSerialNumberState
    {
        ...
        public string GetNextProductionStep(string serialNumber)
        {
            ...
            int error = imsapi.trGetNextProductionStep(sessionContext, init.configHandler.StationNumber, serialNumber, "-1", 0, 1, 0, productionStepResultKeys, out productionStepResultValues);
            if (error != 0)
            {
                string errorMsg = UtilityFunction.GetZHSErrorString(error, init, sessionContext);
