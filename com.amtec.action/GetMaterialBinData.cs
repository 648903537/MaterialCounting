...
namespace com.amtec.action
{
    public class GetMaterialBinData
    {
        ...
        public string[] GetMaterialBinDataDetails(string materialBinNo)
        {
            ...
            LogHelper.Info("end api mlGetMaterialBinData Material bin number:" + materialBinNo + ",result code = " + error);
            if (error == 0)
            {
        ...
        public int ChangeMaterialBinDataQty(string materialBinNo, string qty)
        {
            KeyValue[] materialBinDataUploadValues = new KeyValue[] { new KeyValue("MATERIAL_BIN_QTY_TOTAL", qty) };
            int error = imsapi.mlChangeMaterialBinData(sessionContext, init.configHandler.StationNumber, materialBinNo, materialBinDataUploadValues);
            LogHelper.Info("api mlChangeMaterialBinData (material bin number = " + materialBinNo + ",Qty=" + qty + ",result code = " + error + ")");
            if (error == 0)
            {
                view.errorHandler(0, init.lang.ERROR_API_CALL_ERROR + " mlChangeMaterialBinData " + error, "");
            }
            else
            {
                view.errorHandler(2, init.lang.ERROR_API_CALL_ERROR + " mlChangeMaterialBinData " + error, "");
            }
            return error;
        }
        #endregion
